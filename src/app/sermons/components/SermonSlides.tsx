...
export default function SermonSlidesSection() {
  ...
  // Signup Handler (Auto-login without full page redirect)
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Successfully created and signed in via session
      setShowAuthModal(false);
      setLoading(false);
      if (pendingAction) {
        executePostAuthAction(pendingAction);
          ...
          <div className="bg-slate-950 border border-slate-800 text-white p-6 sm:p-8 rounded-2xl max-w-md w-full relative shadow-2xl my-auto">
            ...
            {authMode === 'login' ? (
              <div className="w-full text-white">
                <h3 className="text-lg font-bold mb-6 text-center">Church Portal Sign In</h3>
                ...
                <form onSubmit={handleLoginSubmit} className="space-y-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-900 border border-slate-800 p-3 rounded-xl text-white text-sm focus:border-blue-500 outline-none" 
                    />
                  </div>
                  <div>
                  ...
                  <button 
                    type="submit" 
                    disabled={loading}
                    className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-800 text-white font-bold p-3 rounded-xl text-xs tracking-widest uppercase transition-all shadow-md mt-4 cursor-pointer"
                  >
